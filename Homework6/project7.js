...
{	
	...
	for(let s of springs){
		...
		const v_i = velocities[i];
		const v_j = velocities[j];

    	const deltaP = p_j.sub(p_i);
    	const deltaV = v_j.sub(v_i);
    	const dist = deltaP.len();
    	const dir = deltaP.div(dist); // unit vector

    	const Fs = dir.mul(stiffness * (dist - s.rest));
    	const Fd = dir.mul(damping * deltaV.dot(dir));
